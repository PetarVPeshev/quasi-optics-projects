...
sgtitle(['Waveguide M_{eq} @ f = ' num2str(wave.f * 1e-9) ' GHz, ' ...
    ...
    17, 'FontWeight', 'bold');
saveas(gcf, 'figures\wg_Meq_silicon.fig');
...
sgtitle(['Waveguide E^{FF} @ f = ' num2str(wave.f * 1e-9) ' GHz, ' ...
    ...
    num2str(round(waveguide.b * 1e3, 2)) ' mm'], 'FontSize', ...
    17, 'FontWeight', 'bold');
saveas(gcf, 'figures\wg_pattern_silicon.fig');
...
saveas(gcf, 'figures\wg_pattern_planes_silicon.fig');

%% SAVE WORKSPACE
wg_field_silicon.sph_grid = sph_grid;
wg_field_silicon.theta = theta;
wg_field_silicon.phi = phi;
wg_field_silicon.waveguide = waveguide;
wg_field_silicon.E = E;
wg_field_silicon.M = M;
wg_field_silicon.E_total = E_total;
save('results\wg_field_silicon.mat', 'wg_field_silicon');
